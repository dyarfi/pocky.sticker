...
class Controller_Backend_RegionProvince extends Controller_Backend_BaseAdmin {
	...
	protected $province;
	protected $parties;
	protected $provinceprovinces;
	protected $user;
	protected $users;
	protected $statuses;

	private $_parent_province;
	...
	public function before () {
		...
		$this->prov			= new Model_Province;
		$this->user			= new Model_User;

		$where_cond			= array('status'	=> 'publish');
		//$this->provs		= $this->prov->find($where_cond);
	...
	function action_index () {
		...
		$total_rows	= $this->prov->find_count($where_cond);
		$listings	= $this->prov->find($where_cond, $order_by, $per_page, $offset);
...
$province_id = $this->id1;
		...
		if ($_POST) {
			...
			if ($post->check()) {
				...
				$last_order	= $this->prov->find_count($where_cond) + 1;
				if (isset($fields['order']) && $fields['order'] < $last_order) {
					$where_cond	= array('province_id'	=> $fields['province_id'],
										'order >='		=> $fields['order']);
					$pages	= $this->prov->find($where_cond);
		...
		$order_by			= array('province_id'	=> 'ASC',
									'order'			=> 'ASC');
		$orders				= $this->prov->find($where_cond, $order_by);
		$content_vars		= array('errors'		=> $errors,
									'fields'		=> $fields,
									'provinceprovinces'	=> $this->provprovinces,
									'orders'		=> $orders,
									'statuses'		=> $this->statuses,
									'class_name'	=> $this->_class_name,
									'class_name' 	=> $this->_class_name,
	...
	public function action_view () {
		...
		/** Views **/
		if ($this->prov->province_id != 0) {
			$province		= $this->prov->load($this->prov->province_id);
			$province		= HTML::chars($province->title, TRUE);
		} else {
			$province		= 'This page doesn\'t have province';
		}
		if ($this->prov->order == 0) {
			$order			= '-';
		} else if ($this->prov->order != 1) {
			$where_cond		= array('province_id'	=> $this->prov->province_id,
									'order'			=> ($this->prov->order - 1));
			$pages		= $this->prov->find($where_cond, '', 1);
			if (isset($pages[0]))
		...
		$where_cond			= array('province_id'	=> $this->prov->id);
		...
		Lib::_auto_image_manipulation($this->_upload_path, $this->file, $this->_prefs);
		/** Views **/
		$content_vars		= array('province'		=> $this->prov,
									'province'		=> $province,
									'files'			=> $files,
									'order'			=> $order,
...
//			/print_r($fields['province_id']); exit(
			...
			if ($post->check()) {
				...
				$where_cond	= array('province_id'	=> $fields['province_id']);
				$last_order	= $this->prov->find_count($where_cond) + 1;
				if ($this->prov->province_id != $fields['province_id']) {
					$where_cond	= array('province_id'	=> $this->prov->province_id,
										'order >='		=> $this->prov->order);
					$orders		= $this->prov->find($where_cond);
					...
					} else if ($this->prov->order < @$fields['order']) {
						$where_cond	= array('province_id'	=> $fields['province_id'],
											'order >'		=> $this->prov->order,
											'order <='		=> $order,
											'status !='		=> 'deleted');
				...
				if ($province_id != '')
					$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->prov->id.'/'.$province_id);
				else
					$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->prov->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
				$errors 	= Arr::overwrite($errors, $post->errors('validation'));
			...
			$fields	= array('province_id'		=> $this->prov->province_id,
							'name'				=> $this->prov->name,
							'subject'			=> $this->prov->subject,
							'description'		=> $this->prov->description,
							'order'				=> $this->prov->order,
	...
	public function action_change() {
		...
		if ($this->request->post('check') !='') {
			...
			foreach ($rows as $row) {
				...
				$this->prov->status	= $this->request->post('select_action');
				$this->prov->update();
			}
	...
	public function _unique_name (Validation $array, $field) {
		if (isset($this->prov->name) && $this->prov->name == $array[$field])
			return;

		$where_cond		= array('name'	=> $array[$field]);
		$name_exists	= ($this->prov->find_count($where_cond) != 0);
	...
	public function _valid_province_id (Validation $array, $field) {
		if ($array[$field] == 0)
			return TRUE;
		$where_cond		= array('id'	=> $array[$field]);
		$parent_exists	= ($this->prov->find_count($where_cond) != 0);
		if (!$parent_exists)
	...
	public function _province_id ($value = '') {
		$where_cond	= array('name LIKE'	=> '%'.$value.'%',
							'status !='	=> 'deleted');
		$buffers	= $this->prov->find($where_cond);
		$ids		= array();
