...
class Controller_Backend_RegionDistrict extends Controller_Backend_BaseAdmin {
	...
	protected $parties;
	protected $province;
	protected $districtprovinces;
	protected $user;
	protected $users;
	protected $statuses;

	private $_parent_district;
	...
	public function before () {
		...
		$this->district			= new Model_District;
		//$this->districtprovince	= new Model_DistrictProvince;
		$this->user				= new Model_User;

		$where_cond					= array('status'	=> 'publish');
		//$this->districts			= $this->district->find($where_cond);
		//$this->districtprovinces	= $this->districtprovince->find($where_cond);
	...
	function action_index () {
		...
		$total_rows	= $this->district->find_count($where_cond);
		$listings	= $this->district->find($where_cond, $order_by, $per_page, $offset);
...
$province_id = $this->id1;
		...
		if ($_POST) {
			...
			if ($post->check()) {
				...
				$last_order	= $this->district->find_count($where_cond) + 1;
				if (isset($fields['order']) && $fields['order'] < $last_order) {
					$where_cond	= array('province_id'	=> $fields['province_id'],
										'order >='		=> $fields['order']);
					$pages	= $this->district->find($where_cond);
		...
		$order_by			= array('province_id'	=> 'ASC',
									'order'			=> 'ASC');
		$orders				= $this->district->find($where_cond, $order_by);
		$content_vars		= array('errors'		=> $errors,
									'fields'		=> $fields,
									'districtprovinces'	=> $this->districtprovinces,
									'orders'		=> $orders,
									'statuses'		=> $this->statuses,
									'module_name'	=> $this->_module_name,
									'class_name' 	=> $this->_class_name,
	...
	public function action_view () {
		...
		$this->district->id	= $id;
		if (!$this->district->load()) {
			$this->redirect(ADMIN.$this->_class_name.'/error/invalid_request');
			return;
		}
		/** Views **/
		if ($this->district->province_id != 0) {
			$province		= $this->province->load($this->district->province_id);
			$province		= HTML::chars($province->title, TRUE);
		} else {
			$province		= 'This page doesn\'t have province';
		}
		if ($this->district->order == 0) {
			$order			= '-';
		} else if ($this->district->order != 1) {
			$where_cond		= array('province_id'	=> $this->district->province_id,
									'order'			=> ($this->district->order - 1));
			$pages		= $this->district->find($where_cond, '', 1);
			if (isset($pages[0]))
		...
		$where_cond			= array('district_id'	=> $this->district->id);
		...
		Lib::_auto_image_manipulation($this->_upload_path, $this->file, $this->_prefs);
		/** Views **/
		$content_vars		= array('district'		=> $this->district,
									'province'		=> $province,
									'files'			=> $files,
									'order'			=> $order,
...
//			/print_r($fields['province_id']); exit(
			...
			if ($post->check()) {
				...
				$where_cond	= array('province_id'	=> $fields['province_id']);
				$last_order	= $this->district->find_count($where_cond) + 1;
				if ($this->district->province_id != $fields['province_id']) {
					$where_cond	= array('province_id'	=> $this->district->province_id,
										'order >='		=> $this->district->order);
					$orders		= $this->district->find($where_cond);
					...
					} else if ($this->district->order < @$fields['order']) {
						$where_cond	= array('province_id'	=> $fields['province_id'],
											'order >'		=> $this->district->order,
											'order <='		=> $order,
											'status !='		=> 'deleted');
				...
				if ($province_id != '')
					$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->district->id.'/'.$province_id);
				else
					$this->redirect(ADMIN.$this->_class_name.'/view/'.$this->district->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
				$errors 	= Arr::overwrite($errors, $post->errors('validation'));
			...
			$fields	= array('province_id'		=> $this->district->province_id,
							'name'				=> $this->district->name,
							'subject'			=> $this->district->subject,
	...
	public function action_change() {
		...
		if ($this->request->post('check') !='') {
			...
			foreach ($rows as $row) {
				$this->district->id	= $row;

				if (!$this->district->load())
					continue;

				$this->district->status	= $this->request->post('select_action');
				$this->district->update();
			}
	...
	public function _unique_name (Validation $array, $field) {
		if (isset($this->district->name) && $this->district->name == $array[$field])
			return;

		$where_cond		= array('name'	=> $array[$field]);
		$name_exists	= ($this->district->find_count($where_cond) != 0);
